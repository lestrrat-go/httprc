...
func (w worker) sendAdjustIntervalRequest(ctx context.Context, r Resource) {
	w.traceSink.Put(ctx, fmt.Sprintf("httprc worker: Sending interval adjustment request for %s", r.URL()))
	select {
	case <-ctx.Done():
	case w.incoming <- adjustIntervalRequest{resource: r}:
	}
	w.traceSink.Put(ctx, fmt.Sprintf("httprc worker: Sent interval adjustment request for %s", r.URL()))
}
